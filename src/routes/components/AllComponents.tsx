...
const AllComponents = () => {
  ...
  const sortData = (sortBy: string) => {
    ...
    const sorted = [...cards].sort((a, b) => {
      ...
      const aValue = sortBy === "name" ? a.name.toLowerCase() : Array.isArray(a[sortBy]) ? a[sortBy][0] : a[sortBy];
      const bValue = sortBy === "name" ? b.name.toLowerCase() : Array.isArray(b[sortBy]) ? b[sortBy][0] : b[sortBy];
    ...
    <GoATable width="100%" onSort={sortData}>
      ...
      {filteredCards.map((card) => (
        <tr key={card.name}>
          ...
          <td>
            <GoABadge
              type={getBadgeType(card.status)} // Determine badge type based on status
              content={card.status} // Display the status as badge content
            />
          </td>
          <td>
            {card.status === "Published" ? (
              ...
              <span>{toSentenceCase(card.name)}</span> // Render as plain text if not "Published"
            )}
          </td>
